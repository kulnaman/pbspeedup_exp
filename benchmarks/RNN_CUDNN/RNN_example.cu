...
int main(int argc, char* argv[]) {
   ...
   printf("Execution Time(s) Forward: %f\n",timeForward*1e-3);
   printf("Execution Time(s) Backward: %f\n",(timeBackward1+timeBackward2)*1e-3);
   printf("Execution Time(s) Total: %f\n",(timeForward+timeBackward1+timeBackward2)*1e-3);
   // Calculate FLOPS
   double forward_FLOPS=numMats * 2ull * (bidirectional ? 2 : 1) * hiddenSize * hiddenSize * seqLength * miniBatch * numLayers / (1e6 * timeForward);
   double backward_FLOPS=numMats * 4ull * (bidirectional ? 2 : 1) * hiddenSize * hiddenSize * seqLength * miniBatch * numLayers / (1e6 * (timeBackward1 + timeBackward2));
   double total_FLOPS=forward_FLOPS + backward_FLOPS;
   ...
   printf("Total GFLOPS: %3.0f GFLOPS \n",total_FLOPS);
   // Calculate FLOPS
