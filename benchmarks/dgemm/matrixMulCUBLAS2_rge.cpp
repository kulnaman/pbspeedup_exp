...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size,
	...
	// CUBLAS version 2.0
	{
		...
		//inserted by R. Ge to test if CPU blocks for each iteration
		struct timeval beginIter, endIter;
		gettimeofday(&beginIter, NULL);
		...
		gettimeofday(&endIter, NULL);
		printf("\ndone with offloading to GPU.\n");
		printf("Time to offload to GPU: %.2f msec\n", (endIter.tv_sec * 1000 + endIter.tv_usec / 1000.0)
				- (beginIter.tv_sec * 1000 + beginIter.tv_usec / 1000.0)
				);
		...
		printf(
				"GPU Performance= %.2f GFlop/s, Time(perIter)= %.3f msec, Time(%dIterations)= %.3f msec, Size= %.0f ps\n",
				gigaFlops, gpu_runtime_iter, nIter, gpu_runtime, flopsPerMatrixMul);
		...
		printf(
				"\nCPU Performance = %.2f GFlops, Time(perIter): %.3lf msec\t Time(%dIterations) = %.3lf msec, # of operations = %.0f ps\n",
				cpu_gflops, cpu_runtime / nIter, nIter, cpu_runtime, flops_on_cpu);
