...
class Experiment:
    def __init__(self, appstr, time, cpu_freq, pkg_pcap, dram_pcap, avg_pkg_pow, avg_dram_pow, gpu_pcap, 
    ...
    @staticmethod
    def csv_header():
        return "appstr,time,cpu_freq,pkg_pcap,dram_pcap,avg_pkg_pow,avg_dram_pow,gpu_pcap,avg_gpu_pow,avg_sm_freq,avg_mem_freq,sm_clock,gpu_mem_clock,sm_offset,mem_offset,spatial_perc,inf,perf"

    def __str__(self):
        return f"{self.appstr},{self.time},{self.cpu_freq},{self.pkg_pcap},{self.dram_pcap},{self.avg_pkg_pow},{self.avg_dram_pow},{self.gpu_pcap},{self.avg_gpu_pow},{self.avg_sm_freq},{self.avg_mem_freq},{self.sm_clock},{self.gpu_mem_clock},{self.sm_offset},{self.mem_offset},{self.spatial_perc},{self.file},{self.perf}"
        ...
        for k, line in enumerate(lines[i:]):
            if "FOOTER" not in line:
                ...
                for dev in range(gpus):
                    ...
                    m_sm_clock.append(int(lsplit[2 + dev * 3]) / 1000.0)
                    m_mem_clock.append(int(lsplit[3 + dev * 3]) / 1000.0)
                base = gpus * 3
                ...
                perf = get_perf(appstr, lines[i:],times[-1])
                break
