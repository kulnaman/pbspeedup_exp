...
public:
	MSRException(unsigned long coreId, int err_no, const char *file_name, int line_no) : mCoreId(coreId), mErrNo(err_no), mLine(line_no), mFile(file_name){}
...
class MSRReader {
public:
	MSRReader(unsigned long core_id);
	MSR_DATA_T readMSRDate(int which);
	void readEnergyData();
	~MSRReader();
	void print();
	unsigned long getCoreId() { return mCoreId; }

	MSRData& getEnergy(){
		mEnergy.pkg = currEnergy->pkg - prevEnergy->pkg;
                if (mEnergy.pkg < 0) 
                {   
                    mEnergy.pkg += (0x01UL<<32) * sPkgEnergyUnits;
                }
		mEnergy.pp0 = currEnergy->pp0 - prevEnergy->pp0;
                if (mEnergy.pp0 < 0) mEnergy.pp0 += double(0x01UL<<32) * sPkgEnergyUnits;
		mEnergy.dram = currEnergy->dram - prevEnergy->dram;
                if (mEnergy.dram < 0) mEnergy.dram += double(0x01UL<< 32) * sDramEnergyUnits;
...
protected:
	double 	sPowerUnits;
	double 	sPkgEnergyUnits;
    double   sDramEnergyUnits;
	double 	sTimeUnits;
...
public:
    ...
    void read();
    MSRData& getEnergyData(int index);
    int getNumCoresSampled();
